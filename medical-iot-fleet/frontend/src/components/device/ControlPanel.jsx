...
export default function ControlPanel({ device, onUpdated }) {
    ...
    return (
        <div className="card space-y-4">
            ...
            <div className={`flex items-center gap-3 rounded-lg border p-3 ${device.is_on
                ? "border-success/30 bg-success/10"
                : "border-border-default bg-surface-2/40"
            }`}>
                <span className={`h-3 w-3 rounded-full ${device.is_on
                    ? "bg-success-light shadow-[0_0_8px_rgba(74,222,128,0.8)]"
                    : "bg-text-disabled"
                }`} />
                <div>
